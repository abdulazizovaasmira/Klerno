...
export default function TeacherRegister({ onRegisterSuccess, onNavigate, initialIsSignIn = false }: TeacherRegisterProps) {
      ...
      try {
        ...
        if (error) {
          ...
          if (isLimitErr) {
            ...
            let finalQualifications = qualFile;
            if (selectedFile) {
              try {
                const extension = selectedFile.name.split('.').pop() || 'pdf';
                const uuid = Math.random().toString(36).substring(2, 15) + Math.random().toString(36).substring(2, 15);
                const filePath = `${userId}/qualifications/profile/${uuid}.${extension}`;
            ...
            const { error: insertErr } = await supabase.from('users').insert(newTeacher);
            if (insertErr) {
              console.error("Supabase teacher fallback profile save failed:", insertErr);
            }
        ...
        const userId = data?.user?.id || `teacher_${Date.now()}`;
        const selectedCountryObj = COUNTRIES.find(c => c.name === country) || COUNTRIES[0];
        ...
        const { error: insertErr } = await supabase.from('users').insert(newTeacher);
        if (insertErr) {
          console.error("Supabase teacher profile save failed:", insertErr);
        }
    ...
    <div className="max-w-3xl mx-auto px-4 py-8" id="teacher-register-root">
      ...
      <div className="text-center space-y-3 mb-8">
        <div className="inline-flex bg-secondary/10 text-secondary border border-secondary/20 px-3 py-1 rounded-full text-xs font-semibold uppercase tracking-wider">
          Teacher Portal {isSignIn ? "Sign In" : "Registration"}
        </div>
        <h2 className="text-3xl font-syne text-[#0c0c10]">{isSignIn ? "Welcome Back to Klerno" : "Teach Globally. Keep 90%."}</h2>
        <p className="text-gray-500 font-medium text-sm md:text-base">
          {isSignIn ? "Access your calendar schedule, messages, and payout details." : "We charge only 10% commission. Onboard in minutes, host lesson slots, and receive payouts securely."}
        </p>
      </div>
      ...
      <div className="bg-white border border-[#e5e5e1] rounded-[14px] p-6 md:p-8 shadow-sm space-y-8">
        ...
        <div className="flex border-b border-[#e5e5e1] mb-6" id="auth-tabs">
          <button
            ...
            className={`flex-1 pb-3 text-sm font-bold border-b-2 transition-colors ${!isSignIn ? 'border-secondary text-secondary' : 'border-transparent text-gray-500 hover:text-gray-700'}`}
            id="tab-signup"
          >
            Sign Up
          </button>
        ...
        {successMsg && (
          <div className="bg-green-50 border border-green-200 text-green-700 font-semibold text-sm rounded-[8px] p-3" id="auth-success-msg">
            ✓ {successMsg}
          </div>
        )}
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          <div className="space-y-4">
            <h3 className="font-syne text-lg text-[#0c0c10] border-b border-[#e5e5e1] pb-2">
              {isSignIn ? "Credentials" : "1. Profile credentials"}
            </h3>
          ...
          <button 
            type="submit"
            disabled={loading}
            className="w-full bg-[#01c9a8] hover:bg-[#01c9a8]/90 text-[#0c0c10] font-bold py-3.5 rounded-[8px] text-sm transition-colors shadow-sm disabled:opacity-50"
            id="teacher-reg-submit"
          >
            {loading ? "Please wait..." : isSignIn ? "Sign In to Teacher Account" : "Create Professional Teacher Account"}
          </button>
        </form>
